...
async function authedContext(browser, viewport, scheme) {
	...
	await ctx2.addInitScript((s) => {
		try {
			localStorage.setItem('ob.color-scheme', s);
		} catch {}
	}, scheme);
	return ctx2;
}
...
async function auditPage(page, label) {
	const result = await page.evaluate(() => {
		...
		for (const el of all) {
			...
			if (r.right > vw + 1)
				out.offscreen.push(
					`${el.tagName}.${(el.className || '').toString().slice(0, 30)} right=${Math.round(r.right)}`
				);
			...
			if (minDim > 0 && minDim < 24 && !isIconLink && !inlineInSentence) {
				out.tinyTargets.push(
					`${el.tagName}"${(el.textContent || el.getAttribute('aria-label') || '').trim().slice(0, 20)}" ${Math.round(r.width)}x${Math.round(r.height)}`
				);
			}
		}
		...
		for (const img of document.querySelectorAll('img')) {
			if (img.complete && img.naturalWidth === 0 && visible(img))
				out.brokenImages.push(img.src.slice(-40));
		}
		...
		const parseCol = (str) => {
			if (!str) return null;
			const srgb = /color\(\s*srgb\s+([\d.]+)\s+([\d.]+)\s+([\d.]+)(?:\s*\/\s*([\d.]+))?\s*\)/.exec(
				str
			);
			if (srgb) {
		...
		const samples = [...document.querySelectorAll('h1, h2, p, a, button, label')]
			.filter((e) => visible(e) && hasDirectText(e))
			.slice(0, 40);
		for (const el of samples) {
			...
			const large =
				...
				(parseFloat(cs.fontSize) >= 18.66 && parseFloat(cs.fontWeight) >= 700);
	...
	for (const scheme of ['night', 'day']) {
		...
		for (const route of ROUTES) {
			...
			try {
				...
				await page.screenshot({
					path: path.join(SHOTS, `${vpName}-${scheme}-${safe}.png`),
					fullPage: true
				});
		...
		if (vpName === 'desktop') {
			...
			try {
				...
				if (!/1\.\s*e4/.test(sheet))
					note(scope, `playing a move failed; scoresheet="${sheet.slice(0, 40)}"`);
				else console.log(`  [ok] ${scope}: e4 played`);
				await page.screenshot({
					path: path.join(SHOTS, `${vpName}-${scheme}-live_game.png`),
					fullPage: true
				});
			} catch (err) {
			...
			try {
				...
				if (mid.stored !== 'day' && mid.stored !== 'night') {
					...
					console.log(
						`  [ok] toggle flipped ${beforeAttr ?? 'night'} -> ${mid.attr}, stored=${mid.stored}`
					);
				}
			} catch (err) {
...
try {
	...
	if (resp.ok()) {
		...
		if (finished.includes('/game/')) {
			...
			console.log(
				`  [info] pgn endpoint on ${id}: ${pgnResp.status()} (${pgnResp.headers()['content-type'] ?? ''})`
			);
		}
	}
	await ctx.close();
